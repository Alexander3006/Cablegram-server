...
import { CreateRoomGrantDto, DeleteRoomGrantDto, RoomGrantDto, UpdateRoomGrantDto } from "./interfaces/roomGrants.dto";

@Controller()
export class RoomGrantsController {
    private readonly _logger: Logger;
    constructor(
        private readonly _roomGrantsService: RoomGrantsService,
    ){
        this._logger = new Logger(RoomGrantsController.name);
    }

    @MessagePattern({cmd: 'create-grant'})
    async createGrant(@Payload() roomGrantDto: CreateRoomGrantDto): Promise<RoomGrantDto> {
        try {
    ...
    async updateGrant(@Payload() roomGrantDto: UpdateRoomGrantDto): Promise<RoomGrantDto> {
        try{
    ...
    async deleteGrant(@Payload() roomGrantDto: DeleteRoomGrantDto): Promise<boolean> {
        try {
